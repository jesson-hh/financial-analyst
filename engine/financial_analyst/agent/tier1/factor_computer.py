...
class FactorComputer(SubAgent[FactorOutput]):
    ...
    async def _execute(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 互通审计 P1⑦:近期落子研判(guanlan 落盘 var/seats_decisions.jsonl)注入研报上下文。
        # 路径:env SEATS_DECISIONS_PATH 优先,缺省 <cwd>/var/seats_decisions.jsonl(报告子进程 cwd=仓根)。
        # 文件缺失/无本票记录 → 空串静默跳过(引擎不强依赖 guanlan 层,best-effort)。
        seats_decisions = ""
        try:
            import json as _json
            import os as _os
            from pathlib import Path as _Path
            _dp = _Path(_os.environ.get("SEATS_DECISIONS_PATH") or (_Path.cwd() / "var" / "seats_decisions.jsonl"))
            if _dp.is_file():
                _rows = []
                for _ln in _dp.read_text(encoding="utf-8").splitlines():
                    try:
                        _r = _json.loads(_ln)
                    except Exception:
                        continue
                    if str(_r.get("code") or "").upper() != str(code).upper():
                        continue
                    if _r.get("kind") == "decide":
                        _rows.append(f"{str(_r.get('ts', ''))[:16]} {_r.get('strategy_name', '')} 研判:"
                                     f"{_r.get('direction', '—')}(置信{_r.get('confidence', '—')})"
                                     f"— {str(_r.get('rationale') or '')[:80]}")
                    elif _r.get("kind") == "order":
                        _rows.append(f"{str(_r.get('ts', ''))[:16]} {_r.get('strategy_name', '')} 条件单:"
                                     f"{_r.get('side', '—')} 止损{_r.get('stop', '—')} 止盈{_r.get('take', '—')}")
                if _rows:
                    seats_decisions = "\n".join(_rows[-5:])   # 最近 5 条,防 prompt 膨胀
        except Exception:
            pass
        ...
        return {
            ...
            "seats_decisions": seats_decisions,
