...
def fetch_1min(client: PytdxClient, code: str, n_bars: int = 480) -> List[dict]:
    """拉最近 n_bars 根 1min K(盘中实时盯盘用). 480 根 ≈ 2 个交易日(一天 240 根).

    与 ``fetch_5min`` 同构,仅 K 线类型换成 1min(pytdx ``KLINE_TYPE_1MIN`` = 8).
    bar dict 形状同 5min(含 ``datetime`` ``YYYY-MM-DD HH:MM``、vol 为**股**,上层转手).
    """
    kt = getattr(TDXParams, "KLINE_TYPE_1MIN", 8)
    if n_bars > 800:
        all_bars: List[dict] = []
        offset = 0
        while offset < n_bars:
            count = min(800, n_bars - offset)
            mkt, c = qlib_code_to_pytdx(code)
            chunk = client.call("get_security_bars", kt, mkt, c, offset, count)
            if not chunk:
                break
            all_bars = chunk + all_bars
            offset += count
        ...
        return all_bars
    mkt, c = qlib_code_to_pytdx(code)
    bars = client.call("get_security_bars", kt, mkt, c, 0, n_bars)
    return bars or []
