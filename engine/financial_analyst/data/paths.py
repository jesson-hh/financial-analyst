...
Priority order (each resolved independently):
  ...
  1. Env vars  — ``FA_QLIB_URI`` / ``FA_PARQUET_ROOT`` / ``FA_NEWS_DATA_ROOT`` / ``FA_PIT_STORE_ROOT``
  2. config/loaders.yaml  — keys ``qlib_binary.{provider_uri, parquet_root, news_data_root, pit_store_root}``
...
class DataPaths:
    ...
    pit_store_root: Path
    """Root directory for the agent-backtest point-in-time store. Contains
    ``{YYYY-MM-DD}/{news,events}.jsonl`` + per-day ``_build.json`` + global
    ``_meta.json``. Written by stocks' ``scripts/build_pit_store.py``; read by
    the fa backtest engine. Line-level jsonl schema is in
    ``docs/data_contract.md`` §8 — fa parses the jsonl directly (no stocks
    import), with an independent dataclass (NOT ``EventItem``)."""
...
def get_data_paths(config_path: Optional[Path] = None) -> DataPaths:
    ...
    # ---- pit_store_root ------------------------------------------------
    # Dev fallback prefers ``parquet_root.parent / pit_store`` (the store lives
    # beside cn_data/parquet under stock_data/), falling back to the static dev
    # root only if parquet_root has no usable parent.
    env_pit = os.getenv("FA_PIT_STORE_ROOT")
    if env_pit:
        pit_store_root = Path(env_pit)
    elif entry.get("pit_store_root"):
        pit_store_root = Path(entry["pit_store_root"])
    elif user_pit.exists():
        pit_store_root = user_pit
    else:
        pit_store_root = parquet_root.parent / "pit_store"
