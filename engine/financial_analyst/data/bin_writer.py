...
_OVERWRITE_ENV = "STOCKS_ALLOW_DATA_OVERWRITE"


class CalendarValidationError(ValueError):
    """Calendar entries must be valid parseable timestamps before writing."""


class ImmutableDataError(RuntimeError):
    """safe_merge_write refused to modify an existing finite historical value."""
...
def _normalize_calendar_entry(value: str, freq: str) -> str:
    entry = str(value).strip()
    if not entry:
        raise CalendarValidationError("calendar entry is empty")
    if freq == "day":
        if not _DAY_RE.match(entry):
            raise CalendarValidationError(f"invalid day calendar entry: {entry!r}")
        ts = pd.to_datetime(entry, errors="coerce", format="%Y-%m-%d")
        if pd.isna(ts) or pd.Timestamp(ts).strftime("%Y-%m-%d") != entry:
            raise CalendarValidationError(f"invalid day calendar entry: {entry!r}")
        return entry
    if not _DATETIME_RE.match(entry):
        raise CalendarValidationError(f"invalid {freq} calendar entry: {entry!r}")
...
def _validate_calendar_entries(calendar: List[str], freq: str) -> List[str]:
    entries = [_normalize_calendar_entry(d, freq) for d in calendar if str(d).strip()]
    if entries != sorted(entries):
        raise CalendarValidationError(f"{freq} calendar entries must be sorted ascending")
    duplicates = len(entries) - len(set(entries))
    if duplicates:
        raise CalendarValidationError(f"{freq} calendar contains {duplicates} duplicate entries")
    return entries


def save_calendar(calendar: List[str], provider_uri: str, freq: str = "day") -> None:
    """Save the full calendar after strict validation.

    Malformed entries are fatal. This protects readers from later crashing on
    corrupted calendar rows such as ``129188-42-``.
    """
    calendar = _validate_calendar_entries(calendar, freq)
    if freq == "day":
    ...
    cal_path = cal_dir / f"{freq}.txt"
    tmp_path = cal_path.with_name(f"{cal_path.name}.tmp.{os.getpid()}")
    try:
        tmp_path.write_text("\n".join(calendar) + "\n", encoding="utf-8")
...
def append_calendar(new_dates: List[str], provider_uri: str, freq: str = "day") -> int:
    ...
    candidates = sorted({
        _normalize_calendar_entry(d, freq)
        for d in new_dates
        if str(d).strip() and str(d).strip() not in existing_set
    })
    if freq == "day":
...
def _allow_data_overwrite() -> bool:
    return os.environ.get(_OVERWRITE_ENV, "").strip().lower() in {"1", "true", "yes", "y"}
...
def _assert_append_only(
    ...
    *,
    ...
    if allow_overwrite or _allow_data_overwrite() or len(old_data) == 0:
        return
    for pos, val in zip(positions, values):
        idx = pos - old_si
        if idx < 0 or idx >= len(old_data):
            continue
        old_val = old_data[idx]
        new_val = float(val)
        if np.isfinite(old_val) and not np.isfinite(new_val):
            raise ImmutableDataError(
                f"refusing to overwrite {instrument}.{field}.{freq} pos={pos}: "
                f"old={old_val!r}, new={new_val!r}. Set {_OVERWRITE_ENV}=1 only for audited repair."
            )
        if np.isfinite(old_val) and np.isfinite(new_val) and not np.isclose(old_val, new_val, rtol=1e-5, atol=1e-6):
            raise ImmutableDataError(
                f"refusing to overwrite {instrument}.{field}.{freq} pos={pos}: "
                f"old={old_val!r}, new={new_val!r}. Set {_OVERWRITE_ENV}=1 only for audited repair."
            )
...
def safe_merge_write(instrument: str, field: str, freq: str, provider_uri: str,
                     positions, values, allow_overwrite: bool = False) -> None:
