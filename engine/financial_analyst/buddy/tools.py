...
def _daily_cached_swarm(slug: str, cmd: list, cn: str, timeout: int = 300) -> ToolResult:
    """日级 swarm 工具共用壳(晨会/主线雷达/海外雷达,审计 N4 档1)。

    这三个工具本质是「每天一份」的产物, 此前每次调用都整段重跑子进程(最长 600s 干等,
    超时只吐一句 timed out)。改为:
      · 当日成功结果落盘 ``~/.financial-analyst/cache/daily_tools/{slug}_{date}.txt``,
        同日再调**秒回**, 文首标「缓存 · HH:MM 生成」诚实显形;
      · 超时 600→timeout(默认 300s), 失败信息带已跑时长;
      · 实时生成失败时, 若有 ≤3 天内旧缓存, 附带返回并**明确标注日期**(不冒充今日)。
    """
    import time as _time
    from datetime import date as _date, datetime as _dtm
    from pathlib import Path as _Path

    cache_dir = _Path.home() / ".financial-analyst" / "cache" / "daily_tools"
    today = _date.today()
    cache = cache_dir / f"{slug}_{today.isoformat()}.txt"
    try:
        if cache.exists():
            txt = cache.read_text(encoding="utf-8")
            ts = _dtm.fromtimestamp(cache.stat().st_mtime).strftime("%H:%M")
            return ToolResult(f"[缓存 · 今日 {ts} 生成]\n{txt}")
    except Exception:  # noqa: BLE001 — 缓存读失败 → 当无缓存, 走真跑
        pass
    ...
    try:
        ...
        return ToolResult(_swarm_fail_msg(slug, cache_dir, today,
                                          f"{cn}超时(上限 {timeout}s)。可稍后重试。"),
                          is_error=True)
    elapsed = int(_time.time() - t0)
    if proc.returncode != 0:
        return ToolResult(_swarm_fail_msg(slug, cache_dir, today,
                                          f"{cn}失败(跑了 {elapsed}s): {proc.stderr[-400:]}"),
                          is_error=True)
...
def _swarm_fail_msg(slug: str, cache_dir, today, head: str) -> str:
    """失败信息: 若有 ≤3 天内旧缓存, 附带返回并明确标注日期(诚实降级, 不冒充今日)。"""
    from datetime import timedelta as _td
    for back in (1, 2, 3):
        day = (today - _td(days=back)).isoformat()
        old = cache_dir / f"{slug}_{day}.txt"
        try:
            if old.exists():
                return (f"{head}\n\n[⚠ 实时生成失败 · 以下为 {day} 的缓存, 仅供参考]\n"
...
def _tool_mainline() -> ToolResult:
    ...
    return _daily_cached_swarm("mainline", ["financial-analyst", "mainline"],
                               "主线雷达", timeout=180)


def _tool_brief() -> ToolResult:
    """Pre-market morning brief (overnight moves + day's watchlist)."""
    return _daily_cached_swarm("brief", ["financial-analyst", "brief"],
                               "盘前晨会", timeout=300)
...
def _tool_overseas_radar() -> ToolResult:
    ...
    """
    return _daily_cached_swarm("overseas_radar", ["financial-analyst", "overseas-radar"],
                               "海外雷达", timeout=300)
...
def _tool_market_status() -> ToolResult:
    """市场状态 (大盘三源): regime (牛/熊/震荡) / 涨停家数 / 主线雷达.

    读 research 端 (收盘后 scripts/export_market_status.py) 写的 market_status.json
    (load 同 watch.market_status 解析, 缺文件/坏 json → 带结构空 dict). 主线为月级信号,
    渲染时带 as-of (调用方据此判断 stale)."""
    from financial_analyst.watch.market_status import load_market_status
    status = load_market_status()
    regime = status.get("regime") or {}
    lu = status.get("limit_ups") or {}
    ml = status.get("mainline") or {}
    # 三源全空 → 文件未生成 (load 容错返回结构占位, 用关键字段判定)
    if not regime.get("regime") and not lu and not ml.get("top") and ml.get("as_of") is None:
        return ToolResult(
            "市场状态未生成, 先在 research 跑 export_market_status.py "
            "(生成 market_status.json 后才有 regime / 涨停家数 / 主线雷达).",
        )
    date = status.get("date")
    lines = [f"市场状态 (as-of {date or '?'}):"]
    # regime (行情)
    rg = regime.get("regime")
    if rg is not None:
        src = regime.get("source")
        bp = regime.get("breadth_pct")
        src_s = f" ({src})" if src else ""
        bp_s = f"  市场宽度 {bp}%" if bp is not None else ""
        lines.append(f"  行情: {rg}{src_s}{bp_s}")
    # 涨停家数
    if lu:
        lines.append(
            f"  涨停 {lu.get('limit_up_total', '?')} "
            f"(主板 {lu.get('limit_up_10', '?')} / 双创 {lu.get('limit_up_20', '?')})  "
            f"跌停 {lu.get('limit_down', '?')}  "
            f"涨/跌 {lu.get('up_count', '?')}/{lu.get('down_count', '?')}"
        )
    # 主线雷达 (月级, 带 as-of)
    top = ml.get("top") or []
    ml_asof = ml.get("as_of")
    lines.append(f"  主线 {ml.get('n_mainline', 0)} 个 (月级, as-of {ml_asof or '?'}):")
    if top:
        for row in top:
            ex = row.get("ex_60d")
            ex_s = f"{ex:+.2f}pp" if isinstance(ex, (int, float)) else "-"
            lines.append(
                f"    {row.get('industry', '?')} "
                f"[{row.get('status', '?')}] 60d 超额 {ex_s}"
            )
    else:
        lines.append("    (当日无主线/启动板块)")
    return ToolResult(
        "\n".join(lines),
        side_effect={"market_status": status},
    )
...
def _tool_stock_brief(code: str, news_days: int = 7) -> ToolResult:
    ...
    # --- Fundamentals (PIT financials — 与因子层同一份防前视财报库) ---
    try:
        fin_res = _tool_financials(code, periods=1)
        if not fin_res.is_error and "无财务数据" not in fin_res.content:
            # 速览里用统一小节头 "## 财务基本面 (PIT, …)", 去掉工具自带 "# {code} 财务基本面" 的 code 冗余
            sec = fin_res.content.replace(f"# {code} 财务基本面", "## 财务基本面", 1)
            lines.append("\n" + sec)
    except Exception:
        pass
...
def _tool_financials(code: str, periods: int = 4) -> ToolResult:
    """个股财务基本面 (PIT 真实财报数据, 非因子).

    读因子层同一份防前视财报 parquet (``loader.fetch_financials`` → as-of/ffill,
    不漏未来财报). 输出: ROE/ROA/净利率, 营收 & 净利 + 同比增速, EPS, 资产负债率,
    净资产, 经营现金流; 含近 ``periods`` 期趋势. 小盘/次新若财报库未覆盖 → 诚实返回
    无数据 (而非编造). 研究域工具 — 它取的是原始财报口径, 不是因子 IC/回测, 故不归
    量化模块.
    """
    try:
        from financial_analyst.data.loader_factory import get_default_loader
    ...
    except Exception as exc:  # noqa: BLE001 — 取数失败诚实报错, 不崩整轮
        return ToolResult(f"financials 取数失败 ({type(exc).__name__}: {exc})", is_error=True)
    if fin is None or len(fin) == 0:
        return ToolResult(
            f"{code} 无财务数据 (PIT 财报库未覆盖该股, 多为次新/小盘/停牌/B股). "
            f"行情/估值仍可用 quote_lookup."
        )

    fin = fin.tail(max(1, int(periods)))

    def _pct(v):
        try:
            f = float(v)
            return "—" if f != f else f"{f * 100:.2f}%"   # f!=f → NaN
        except Exception:
            return "?"
    ...
    def _yi(v):
        try:
            f = float(v)
            return "—" if f != f else f"{f / 1e8:.2f}亿"
        except Exception:
            return "?"
    ...
    last = fin.iloc[-1]
    ann = str(fin.index[-1])[:10]
    lines = [f"# {code} 财务基本面 (PIT, 最新公告 {ann})"]
    lines.append(
        f"  ROE={_pct(last.get('roe'))}  ROA={_pct(last.get('roa'))}  净利率={_pct(last.get('net_margin'))}\n"
        f"  营收={_yi(last.get('revenue'))} (同比 {_pct(last.get('rev_yoy'))})  "
        f"净利={_yi(last.get('net_income'))} (同比 {_pct(last.get('np_yoy'))})\n"
        f"  EPS={_f2(last.get('eps'))}  资产负债率={_pct(last.get('debt_ratio'))}  "
        f"净资产={_yi(last.get('total_equity'))}  经营现金流={_yi(last.get('cfo'))}"
    )
    if len(fin) > 1:
        lines.append("\n## 近几期趋势 (旧→新)")
        for dt, r in fin.iterrows():
            lines.append(
                f"  [{str(dt)[:10]}] ROE={_pct(r.get('roe'))} "
                f"营收={_yi(r.get('revenue'))}(YoY {_pct(r.get('rev_yoy'))}) "
                f"净利={_yi(r.get('net_income'))}(YoY {_pct(r.get('np_yoy'))}) "
                f"EPS={_f2(r.get('eps'))}"
            )
    return ToolResult("\n".join(lines))
...
def _tool_quant_reports(query: Optional[str] = None, top_k: int = 5) -> ToolResult:
    """列出觀瀾工作流报告库的最近验证报告(因子 IC / 回测 KPI 快照),供研判引用。

    P2-F(互通审计):报告库此前只有工作流页自己能看。guanlan 层产物,路径经
    GUANLAN_REPORTS_DIR 覆盖,缺省 <仓根>/guanlan_v2/reports/store;缺目录诚实空。
    """
    import json as _json
    _root = os.environ.get("GUANLAN_REPORTS_DIR")
    d = Path(_root) if _root else (_project_root() / "guanlan_v2" / "reports" / "store")
    if not d.is_dir():
        return ToolResult("工作流报告库为空(目录不存在)。")
    rows: List[str] = []
    for p in sorted(d.glob("*.json"), reverse=True):          # 文件名以 UTC 时间戳开头 → 逆序=最新在前
        try:
            rec = _json.loads(p.read_text(encoding="utf-8"))
        except Exception:  # noqa: BLE001 — 坏文件跳过
            continue
        name = str(rec.get("name") or rec.get("workflow_name") or p.stem)
        if query and str(query) not in name:
            continue
        kpi = rec.get("kpi") or {}
        if not kpi:                                            # kpi 前端选填;空则回退 result 真指标
            res = rec.get("result") or {}
            _ic = res.get("ic") or {}
            _pf = res.get("portfolio") or {}
            kpi = {k: v for k, v in {
                "rank_ic": _ic.get("rank_ic_mean"), "icir": _ic.get("icir"),
                "sharpe": _pf.get("sharpe"),
                "ann_return": _pf.get("net_ann", _pf.get("ann_return")),
                "max_drawdown": _pf.get("max_drawdown")}.items() if v is not None}
        kpis = " ".join(f"{k}={v:.4f}" if isinstance(v, float) else f"{k}={v}" for k, v in kpi.items())
        rows.append(f"- {rec.get('ts', '')} 《{name}》 池={rec.get('universe', '—')} "
                    + (kpis or "(无KPI快照)"))
        if len(rows) >= max(1, int(top_k)):
            break
    if not rows:
        return ToolResult("工作流报告库无匹配报告。")
    return ToolResult("觀瀾工作流验证报告(最新在前;数值为真实回测/IC 快照,引用时注明出处):\n"
                      + "\n".join(rows))
...
TOOL_REGISTRY: List[Tool] = [
    ...
    Tool(
        name="financials",
        description=(
            "个股财务基本面 (PIT 真实财报数据, 非因子): ROE/ROA/净利率, 营收 & 净利 + 同比增速, "
            "EPS, 资产负债率, 净资产, 经营现金流; 含近几期趋势. 与因子层同一份防前视(as-of)财报库. "
            "Use when the user asks 基本面/财务/ROE/营收/净利/成长/负债, 或给「XX 怎么样」补基本面. "
            "小盘/次新若财报库未覆盖会诚实返回无数据."
        ),
        input_schema={
            "type": "object",
            "properties": {
    ...
    Tool(
        name="market_status",
        description=(
            "大盘市场状态 (三源): regime (牛/熊/震荡 + 市场宽度) / 涨停家数 "
            "(总/主板/双创/跌停/涨跌家数) / 主线雷达 (月级 top 板块 + 60d 超额, 带 as-of). "
            "Use when user asks 今天大盘怎么样 / 行情如何 / 涨停家数 / 主线 / 题材 / 市场情绪. "
            "Market regime, limit-up breadth, and monthly mainline sectors for the whole A-share market."
        ),
        input_schema={"type": "object", "properties": {}},
        run=_tool_market_status,
        cost_hint="instant",
    ...
    Tool(
        name="quant_reports",
        description=(
            "列出觀瀾工作流报告库的最近因子验证/回测报告(真实 RankIC/Sharpe/回撤快照). "
            "用户问『我验证过哪些因子/工作流结论』或研判想引用量化验证结果时用. "
            "query 可按报告名过滤."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "可选: 报告名子串过滤"},
                "top_k": {"type": "integer", "default": 5},
            },
        },
        run=_tool_quant_reports,
        cost_hint="instant",
        confirm_required=False,
    ),
]
...
# ── 模块工具域 (guanlan 模块边界) ──────────────────────────────────────────────
# 引擎只提供机制 (按名过滤可调用工具); 由 /run 的 ``profile`` / ``tools`` 参数选用。
# 对话/研报模块 = 研究域 (research): 行情/资金流/新闻/产业链/研报/扫描/经验, 不含因子炼制。
# 因子炼制/评测归量化模块 (factor)。见 docs/module_map.md。
FACTOR_TOOL_NAMES = frozenset({
    "alpha_forge", "factor_test", "alpha_compare", "factor_report",
    "factor_compose", "user_factors", "alpha_bench", "alpha_snapshot",
    "alpha_list", "alpha_show", "event_report",
})


def profile_tool_names(profile: Optional[str], explicit=None) -> Optional[set]:
    """把模块 profile 名解析成「可调用工具名集合」(None = 不限制)。

    - ``explicit`` (显式白名单 list/set) 优先: 直接用它 ∩ TOOL_REGISTRY 真实名。
    - ``'research'`` (对话/研报域): 全部工具 − 因子类。
    - ``'factor'`` (量化域): 因子类 + 通用行情/查询 (量化也要看行情/产业链)。
    - ``'all'`` / None / 未知: 全部工具 − ww_*(旧契约返回 None=完全不限制,
      会把帷幄工具一并外露;改返具体集合,对 /run、/tools 的过滤口径等价)。
    """
    allnames = {t.name for t in TOOL_REGISTRY}
    if explicit:
        return {n for n in explicit if n in allnames}
    # ww_* 是帷幄 console 专属(经 CONSOLE_ALLOWED 显式白名单走),profile 路径一律不外露——堵已退役 chat 页 /run 直链的越权面
    allnames = {n for n in allnames if not n.startswith("ww_")}
    if profile == "research":
        return allnames - set(FACTOR_TOOL_NAMES)
    if profile == "factor":
        common = {"realtime_quote", "quote_lookup", "quote_batch", "stock_brief",
                  "chain_for", "industry_show", "iwencai_search", "news_query",
                  "financials"}
        return (set(FACTOR_TOOL_NAMES) | common) & allnames
    return allnames   # 缺省 / 'all' / 未知 profile:全部 − ww_*
