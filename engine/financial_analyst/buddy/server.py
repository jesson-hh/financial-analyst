...
class RunReq(BaseModel):
    ...
    # 模块工具域 (guanlan 模块边界): 'research'(对话/研报域=全部−因子) / 'factor'(量化域) /
    # None=不限制。对话端传 'research' → agent 看不到也调不到因子类工具。见 buddy.tools.profile_tool_names。
    profile: Optional[str] = None
    tools: Optional[list] = None        # 显式工具白名单 (覆盖 profile); None=按 profile
    # 抗重启记忆: 客户端持有的近期对话 [{role:'user'|'assistant', content}]。后端 in-memory
    # 会话历史会被进程重启(看门狗/崩溃/部署)清空 → agent 变冷。冷 agent 用它 seed 回历史,
    # 让多轮记忆跨重启/刷新/换实例存活。热 agent(本进程已有真历史)忽略它,不重复注入。
    history: Optional[list] = None


def _seed_agent_history(agent, history, max_msgs: int = 16, max_chars: int = 8000) -> None:
    """用客户端带来的近期对话给「冷」agent(刚重启、``messages`` 为空)注入历史。

    只收 ``role in {user, assistant}`` 且 content 为非空字符串的轮次(tool 原文/空消息剔除);
    末尾按 ``max_msgs`` / ``max_chars`` 双重截断, 保留最近的。调用方负责仅在 ``agent.messages``
    为空时调用(热 agent 不该被覆盖)。``history`` 为空 → no-op(即旧行为)。
    """
    ...
    while len(clean) > 1 and sum(len(c) for _, c in clean) > max_chars:
        clean.pop(0)
...
def build_app():
    ...
    from financial_analyst.buddy.recipes import get_recipe, run_recipe, RecipeFallback
    ...
    async def run(body: RunReq):
        ...
        mode = body.mode
        # 模块工具域: 对话端传 profile='research' → agent 只见研究类工具 (因子类被裁掉);
        # body.tools 显式白名单优先。None=不限制 (向后兼容)。
        from financial_analyst.buddy.tools import profile_tool_names
        allowed_tools = profile_tool_names(body.profile, body.tools)
        ...
        async def produce():
            # idx is shared across recipe + run_turn so tool ordering on the
            # SSE stream stays monotonic regardless of which engine ran.
            idx = -1

            async def forward(evt):
                # Translate a TurnEvent into SSE frames. Used identically by the
                # recipe path and the free run_turn loop, so fallback behavior is
                # byte-for-byte the same as before the recipe fork existed.
                nonlocal idx
            ...
            try:
                ...
                # 抗重启记忆: 进程重启清空 in-memory sessions → 冷 agent (messages 空)。用客户端
                # 随每轮带上的近期对话 seed 回历史 (仅当冷; 热 agent 已有真历史, 不覆盖/不重复)。
                if not agent.messages and body.history:
                    _seed_agent_history(agent, body.history)
                intent = classify(query)
                recipe = get_recipe(intent)
                ...
                if recipe is not None:
                    try:
                        async for evt in run_recipe(recipe, query, body.context,
                                                    agent._client, confirm_cb,
                                                    agent=agent):
                            await forward(evt)
                        ran = True
                    except RecipeFallback:
                        ran = False  # slot 解析失败 → 落自由循环
                if not ran:
    ...
    # `/lesson <text>`. Written to memories/_buddy/conversation_lessons.md (OUT of
    # _shared/ so it is NOT force-loaded into report agents); buddy reads it live on
    # every prompt build. Closes the "persist conversation experience" loop.
    @app.post("/lesson")
    async def lesson(body: LessonReq):
        text = (body.text or "").strip()
        if not text:
            return JSONResponse({"ok": False, "error": "empty text"})
        from financial_analyst.buddy.agent import _conversation_lessons_path
        ...
        if not f.exists():
            f.write_text(
                ...
                "用户在 buddy 对话里 `/lesson <text>` 沉淀的经验. buddy 每次构建 prompt 实时载入.\n",
                encoding="utf-8",
            )
    ...
    @app.get("/tools")
    async def tools(profile: Optional[str] = None):
        # ?profile=research → 只列研究类 (对话端 ⌘K 用); ?profile=factor → 因子类; 缺省=全部。
        from financial_analyst.buddy.tools import TOOL_REGISTRY, profile_tool_names
    ...
    @app.get("/watch/signal_pack")
    async def watch_signal_pack(date: Optional[str] = None, top_n: int = 20):
        """今日 EOD 信号包: rows=当日全量, pool=盯盘池候选 (剔负向 + combo 排序).

        research 端 (收盘后) 写 daily_signal_pack.parquet, fa 端 (盘前/盘中) 读.
        缺文件 → 空 (HTTP 200, 同 history 不 500). pool seed 供盯盘台 P4 直接消费.
        """
        try:
            from financial_analyst.watch.signal_pack import load_signal_pack, pack_to_pool
            df = await asyncio.to_thread(load_signal_pack, None, date)
            pool = pack_to_pool(df, top_n=int(top_n or 20))
            rows = _jsonable(df.to_dict("records"))
            return JSONResponse({"ok": True, "n": len(rows),
                                 "pool": [{"code": it.code} for it in pool],
                                 "rows": rows})
        except Exception as exc:  # noqa: BLE001
            return JSONResponse({"ok": False, "n": 0, "pool": [], "rows": [],
                                 "reason": f"{type(exc).__name__}: {exc}"})

    @app.get("/watch/market_status")
    async def watch_market_status():
        """市场状态条 (P4.1-full): regime (牛/熊/震荡) / 涨停家数 / 主线雷达 三市场级源.

        research 端 (收盘后, scripts/export_market_status.py) 写 market_status.json, fa 读.
        缺文件 → 带结构空 dict (HTTP 200 不 500, 同 signal_pack). 供盯盘台 MarketStatusBar 消费.
        """
        try:
            from financial_analyst.watch.market_status import load_market_status
            status = await asyncio.to_thread(load_market_status, None)
            return JSONResponse({"ok": True, **_jsonable(status)})
        except Exception as exc:  # noqa: BLE001
            return JSONResponse({"ok": False, "reason": f"{type(exc).__name__}: {exc}",
                                 "date": None, "regime": {}, "limit_ups": {},
