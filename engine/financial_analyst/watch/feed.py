...
class WatchFeed:
    ...
    def bars1(self, code: str, n: int = 480) -> pd.DataFrame:
        """拉近 ``n`` 根 1min K, 转 DataFrame(列同 bars5, vol 已 ÷100 转手).

        盘中实时盯盘用(更细粒度). 退市/拉空/网络失败 → 列齐的空 DataFrame(不抛).
        """
        try:
            from financial_analyst.data.updaters.pytdx_kline import fetch_1min
            bars = fetch_1min(self._get_client(), code, n_bars=n)
        except Exception as e:  # noqa: BLE001 — 容错
            log.warning("WatchFeed.bars1 fetch failed for %s: %s", code, e)
            return _empty_bars()
        ...
        for b in bars:
            try:
                ...
                log.debug("WatchFeed.bars1 skip bad bar for %s: %s (%r)", code, e, b)
                continue
