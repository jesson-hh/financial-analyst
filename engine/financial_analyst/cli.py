...
def report(
    ...
    for c in codes:
        try:
            ok = asyncio.run(run_report_oneshot(code=c, asof=asof, out_dir=out_dir, trace=trace))
            if ok:
                completed += 1
            else:
                failed += 1
                typer.echo(f"[failed] {c}: 研报生成失败(上游 agent 未全部成功)")
    ...
    # 有任一份研报失败 → 非零退出码,让子进程调用方(帷幄 _call_buddy_report)能识别失败,
    # 不再被 exit 0 误判成功而取历史旧档冒充。成功的(completed)已各自落盘,不受影响。
    if failed:
        raise typer.Exit(code=1)
