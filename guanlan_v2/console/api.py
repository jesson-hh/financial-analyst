...
另有:深度研报 ww_report_run(后台5-8分钟,需确认)、调界面 ww_show_page(用户说『调出/打开/看看XX界面』就用它)、沉淀经验卡 ww_cards_save(需确认)、长期记忆 ww_memory_write/ww_memory_read、哨兵研判历史 ww_seats_history(查落子哨兵的研判/条件单记录,全局跨会话)、创建盯盘 agent ww_seats_bind(为某票在校场建专属盯盘 agent,需确认)、消息面 ww_news_search(个股/大盘实时新闻+情绪)、现拉实时新闻 ww_news_live(单票秒回,个股新闻+快讯+stocks公告政策富层,不编造)、stocks 实时文本探针 ww_live_text(13端点:巨潮公告/互动易问答/个股·行业研报元数据/涨停池·涨停原因·热点归因/热榜·人气榜·热门概念/概念归属,source=catalog 列全部端点,缺料诚实空不编造)、全球情绪温度计 ww_macro_pulse(Polymarket+Kalshi 预测市场概率观测全球宏观预期:降息/衰退/地缘/中美/加密五主题温度 + A股打板温度;纯展示型参考绝非交易信号,问『全球宏观预期/降息概率/情绪冷热』时用)。
...
7. 用户问个股/大盘"最近消息面/新闻情绪/有什么新闻"→ 调 ww_news_search(实时东财快讯+情绪,带引用,无则诚实标注);问"此刻这只票有什么新闻/现在有什么消息"要秒回现拉→调 ww_news_live;问"最新公告/互动易回复/研报评级/今天涨停梯队·原因/什么题材热/人气榜"→ 调 ww_live_text(选对应 source,不确定先 source=catalog)。
