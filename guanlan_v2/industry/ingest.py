...
async def _run_batch(docs: list, fw: dict, client) -> dict:
    ...
    fw_id = str(((fw.get("meta") or {}).get("id")) or "ai_chain")
    ...
    async def _one(doc: dict):
        async with sem:
            ...
            if r.get("ok"):
                await asyncio.to_thread(store.append_extraction, r["extraction"], fw_id)
                totals["n_ok"] += 1
...
def _worker(limit: Optional[int], client, backfill_days: Optional[int] = None,
            fw_id: str = "ai_chain") -> None:
    ...
    try:
        ...
        scan = corpus.scan_new_docs(wm, all_pool_codes(fw), _extract_keywords(fw),
                                    limit=limit, exclude_doc_ids=store.load_extracted_doc_ids(fw_id),
...
def start_ingest(limit: Optional[int] = None, client=None, backfill_days: Optional[int] = None,
                 fw_id: str = "ai_chain") -> dict:
    global _running
    with _run_lock:
        if _running:
            return {"ok": True, "accepted": False, "running": True, "reason": "已有批处理在跑(单飞)"}
        _running = True
    t = threading.Thread(target=_worker, args=(limit, client, backfill_days, fw_id),
                         daemon=True, name="industry-ingest")
    t.start()
    return {"ok": True, "accepted": True, "running": True, "reason": None}


def ingest_state(fw_id: str = "ai_chain") -> dict:
    from . import store
    st = store.load_state(fw_id)
    st["running"] = _running
