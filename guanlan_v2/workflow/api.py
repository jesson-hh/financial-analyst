...
- feature   | feat:series, label:series(可选), src:series(接 source.data,下传股票池) | fe:fe | tag — 把上游公式物化成真 X/y;fe 块是 ML/PCA 入口
...
6. feature.feat 由 formula/factorlib(因子)喂;**feature.src 必须接 source.data**(下传股票池/样本外占比,缺则退回默认池 csi_fast);feature.label 可选。每张图必须有一个 source 且其 data 连到某 feature.src。
7. 同一个(节点, 入端口)最多一条入边;不要自连(from 节点 === to 节点)。
...
def validate_graph(graph: Any) -> "tuple[bool, List[str]]":
    ...
    for eidx, edge in enumerate(edges):
        ...
        # 7. single-in-edge(对齐 workflow.jsx:601 单入口)。
        key = (dst_id, dst_port)
