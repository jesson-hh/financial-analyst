...
5. `RunEvent` accepts `SchemaRef + PayloadRef`; Phase 1 `TypedPayloadRef` preserves exact schema/content with audit-only object relocation whenever a public/runtime fact must carry both; `Artifact`, `InputSnapshot`, `NodeRun`, `BudgetReservation`, `PlanApproval`, `PlanValidationReport` and `StaticLegacyPlanAttestation` reject arbitrary digest strings;
...
**Consumes:** Phase 1 `SchemaRegistry`, `SchemaRef`, `PayloadRef`, `TypedPayloadRef`, `RunEvent`, `EventCursor`, `LayerCommit` and their pure builders, plus Task 1 `BudgetLedger`, `BudgetEventSink` and closed `BudgetTransitionCommand`.
...
- same-batch nested-control/event-target tests use `StagedPayloadRef`/`StagedTypedPayloadRef` sentinels, prove commit returns final refs/events without exposing a pre-commit object ID, and reject unknown/duplicate/cross-batch/cyclic keys, wrong schema/namespace, fabricated locators and semantic drift on retry.
...
**Consumes:** Phase 1 `Artifact`, `ArtifactRef`, `InputArtifactBinding`, `InputSnapshot`, `NodeRun`, `LayerCommit`, `TypedPayloadRef`, their builders/validators, sealed SchemaRegistry, plus the Task 2 stores.
...
- `ArtifactPool`, scoped to one run and frozen Plan:
  ...
  - `freeze_input_snapshot(node, *, run_id, plan, layer_index, attempt, context_snapshot_ref: TypedPayloadRef, bound_artifact_inputs: tuple[InputArtifactBinding, ...], data_result_refs: tuple[TypedPayloadRef, ...] = (), memory_record_refs=(), readiness, missing_input_names=()) -> InputSnapshot`;
  - `replay(...) -> ArtifactPool`.
...
10. a `ready` InputSnapshot contains exactly the selected node's named Artifact inputs available **before node start**. `one` has one full ArtifactRef; `many` preserves Plan dependency declaration order; outer bindings follow WorkerSpec declaration order. Pre-existing Context and DataResult evidence enter only as exact main TypedPayloadRefs, while memory enters as full canonical MemoryRecordRefs;
...
12. DataResult TypedPayloadRefs obtained during node execution do not mutate or replace the pre-node InputSnapshot; they bind NodeRun `data_result_refs`/ToolCallRecords and successful Artifact Provenance instead;
...
- late stage fails;
- complete ready/terminal-partial InputSnapshot field matrix, one/many binding order, exact context/DataResult TypedPayloadRefs and execution rejection of terminal-partial;
...
- [ ] every pre-Phase-3 ContextSnapshot uses the two persisted canonical empty-memory snapshot/selection main `TypedPayloadRef`s, each hash equals its nested payload content digest, `memory_session_id=None` and `runtime_requirements_ref=None`; no caller/worker session field is invented.
