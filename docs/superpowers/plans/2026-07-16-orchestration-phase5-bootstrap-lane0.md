...
- **The reviewed v1 factor set** (hand-frozen in `market_factor_set_v1.json`; `factor_set_version="mfs-v1"`, `feature_schema_version="mfs-v1"`, `universe="all_a"`) — exactly 15 factor ids mapping the spec §5 table's 12 rows onto ids (a row carrying multiple factors — 涨停强度/炸板率, 连板高度/晋级率, 已实现波动率/估值分位 — yields one id per factor). Windows/params are the spec's stated defaults; where the spec leaves a parameter "待设计", the v1 value below is the reviewed provisional (tuning belongs to Phase 4 `run_optimize` over matured cases — spec §5 "参数不拍脑袋" closed loop, out of scope here):

| factor_id | spec §5 row | v1 params | aux_keys | required_inputs | min_history_sessions |
|---|---|---|---|---|---|
| `breadth.ad_ratio` | 涨跌家数比 AD | `ma_short=5, ma_long=20, slope_window=5` | `ma5, ma20, slope` | `updown` | 25 |
| `breadth.nh_nl` | 新高新低差 | `window_short=20, window_long=60` | `nhnl20, nhnl60` | `closes_panel` | 61 |
| `breadth.limit_up_ema` | 涨停强度 | `ema_span=3` | `ema3` | `limit_up_total` | 4 |
| `breadth.break_rate` | 炸板率 | `-` (definitional: `zb/(zt+zb)`) | `-` | `break_counts` | 5 |
| `breadth.ladder_height` | 连板高度 | `-` (最高连板) | `-` | `board_pools` | 5 |
| `breadth.promotion_rate` | 晋级率 | `numerator="limit_days>=2"` (今日≥2连板家数/昨全涨停池 — the implemented `market_tape.promotion_rate` 口径, reused as-is; 严格首板口径 deferred with the pool history) | `-` | `board_pools` | 5 |
| `breadth.divergence` | 广度背离 ★ | `z_window=20, ret_window=20, breadth_series="ad_ratio_ma20", top_threshold=1.5` | `z_index, z_breadth` | `updown, closes_index` | 41 |
| `flow.north_trend` | 北向趋势 | `cum_short=5, cum_long=20, pct_window=250` | `cum5, cum20, slope, pct250` | `north_net` | 250 |
| `flow.main_net_pct` | 主力净额分位 | `pct_window=250` | `pct250` | `main_net` | 250 |
| `rotation.hhi` | 板块资金集中度 | `method="top3_share"` | `hhi` | `sector_flows` | 20 |
| `rotation.mainline_diffusion` | 主线扩散度 | `top_mainlines=3` | `-` | `mainline_membership, closes_panel` | 20 |
| `rotation.industry_dispersion` | 行业动量离散度 | `ret_window=20, dispersion="cross_sectional_std"` | `-` | `industry_returns` | 21 |
| `vol.rv20` | 已实现波动率 | `window=20, long_window=60` | `rv_ratio` | `closes_index` | 61 |
| `val.index_pe_pct` | 估值分位 | `pct_years=5` | `pe_pct, pb_pct` | `index_valuation` | 1150 |
| `temp.astock` | 打板温度 | `-` (已有 market_tape 温度公式产出的历史点) | `-` | `astock_temp` | 20 |

Expected v1 production outcome (documented, not hard-coded — the classification is data-probed at run time per Task 3): computable today — `breadth.ad_ratio`, `breadth.nh_nl`, `breadth.limit_up_ema`, `breadth.divergence`, `vol.rv20`; short-history/accreting — `temp.astock` (macro snapshots jsonl accretes since 2026-07-06); honest UNAVAILABLE until a history store exists — `breadth.break_rate`, `breadth.ladder_height`, `breadth.promotion_rate` (board-pool history absent; `market_tape` is snapshot-only), `flow.north_trend` (no daily store; 北向 2024-08 停披), `flow.main_net_pct`, `rotation.hhi`, `rotation.mainline_diffusion`, `rotation.industry_dispersion`, `val.index_pe_pct` (only per-stock live percentile exists). This resolves the spec §13 "Lane 0 数据覆盖" open item by construction: whatever lacks strict-replay coverage stays UNAVAILABLE.
- `class MarketFactorValue(DigestModel)` (registered `MarketFactorValue@1`): `schema_version: Literal["1"] = "1"`, `factor_id: LogicalId`, `definition_version: NonEmptyStr`, `value: FiniteFloat | None` (spec field: the latest point's value; `None` iff `status == "unavailable"`), `params: dict[NonEmptyStr, FiniteFloat | NonEmptyStr]`, `universe: NonEmptyStr`, `frequency: Literal["day"] = "day"`, `effective_at: UtcDateTime` (spec field: the newest contributing observation's session time; equals `available_at` when unavailable), `available_at: UtcDateTime`, `status: Literal["ok", "unavailable"]` (the closed spec literal — partiality is carried by `coverage`, never by a third status value), `coverage: FiniteFloat` (ge=0, le=1), `missing_policy: Literal["unavailable"] = "unavailable"`, `points: tuple[MarketFactorPoint, ...] = ()` (additional series field; strictly increasing dates), `unavailable_reason: NonEmptyStr | None = None`, `content_digest: DigestHex` (SELF_DIGEST). Status matrix: `"ok"` ⇒ non-empty points, `value` equals the last point's value, `0 < coverage <= 1`, reason **required** iff `coverage < 1` (partial window) and forbidden at `coverage == 1.0`; `"unavailable"` ⇒ empty points, `value is None`, `coverage == 0.0`, reason required.
- `class MarketFactorReport(DigestModel)` (registered `MarketFactorReport@1`): `schema_version: Literal["1"] = "1"`, `as_of: UtcDateTime`, `factor_set_version: NonEmptyStr`, `factor_set_digest: DigestHex`, `feature_schema_version: NonEmptyStr`, `universe: NonEmptyStr`, `values: tuple[MarketFactorValue, ...]` (spec field name; sorted by `factor_id`; ids exactly equal the bound set's ids), `data_snapshot_hash: DigestHex` (spec field: digest of the exact PIT input snapshot the report was computed from — bound by Task 3's compute core to the windowed `MarketFactorInputs` content digest), `coverage: FiniteFloat` (ge=0, le=1; spec field: report-level coverage = mean of per-factor `coverage` over the bound set — formula pinned by tests), `feature_vector: dict[LogicalId, FiniteFloat]` (latest point per `ok` factor — the exact dict the experience layer consumes), `feature_coverage: dict[LogicalId, FiniteFloat]`, `missing_features: tuple[LogicalId, ...]` (sorted; exactly the `unavailable` factor ids), `unavailable_factor_ids: tuple[LogicalId, ...]` (== `missing_features`; kept for report readers), `badges: tuple[NonEmptyStr, ...] = ()`, `content_digest: DigestHex` (SELF_DIGEST). Cross-consistency validators tie `feature_vector`/`feature_coverage`/`missing_features`/`coverage` to the `values` tuple bit-for-bit.
...
3. an `unavailable` factor can never carry points, a non-None `value` or a nonzero coverage (zero-fill structurally impossible);
...
- `MarketFactorValue` status matrix — each illegal combination (ok+empty points, ok+`value=None`, ok+coverage==0, ok+coverage<1 without reason, ok+coverage==1 with reason, unavailable with points, unavailable with non-None `value`, unavailable+coverage>0, any string outside the closed `ok|unavailable` literal) rejected; each legal combination accepted;
...
- `MarketFactorSetSpec` sorted/dup-free/non-empty definitions; golden digest reproduction (load `market_factor_set_v1.json`, rebuild, compare `content_digest`); the golden's 15 factor ids asserted verbatim;
- `MarketFactorReport` cross-consistency both directions: a `feature_vector` key for an `unavailable` factor rejected; an `ok` factor absent from `feature_vector` rejected; `missing_features` ≠ `unavailable` ids rejected; ids not matching the bound set rejected; report `coverage` deviating from the pinned mean-of-per-factor-coverage formula rejected;
...
**Consumes:** Phase 1 `Confidence`, `RotationStage` (enums.py:31 — `START="启动"`, `SPREAD="扩散"`, `DIVERGENCE="分化"`, `EBB="退潮"`, `UNKNOWN="unknown"`), Task 1 types.

**Produces:**

- Axis label constants: `TREND_LABELS = ("bull", "bear", "range", "unknown")`, `RISK_LABELS = ("risk_on", "risk_off", "neutral", "unknown")`, `HEAT_LABELS = ("normal", "overheat", "unknown")`; tolerance/gating constants `AXIS_SUM_TOLERANCE = 1e-8`, `UNKNOWN_ATTENTION_THRESHOLD = 0.25`, `HIGH_CONFIDENCE_UNKNOWN_MAX = 0.10`.
- `class RegimeReport(DigestModel)` (registered `RegimeReport@1`): `schema_version: Literal["1"] = "1"`, `as_of: UtcDateTime`, `trend: Literal["bull","bear","range","unknown"]`, `risk_state: Literal["risk_on","risk_off","neutral","unknown"]`, `heat_state: Literal["normal","overheat","unknown"]` (spec modal fields), `trend_probabilities: dict[Literal["bull","bear","range","unknown"], FiniteFloat]`, `risk_probabilities: dict[Literal["risk_on","risk_off","neutral","unknown"], FiniteFloat]`, `heat_probabilities: dict[Literal["normal","overheat","unknown"], FiniteFloat]`, `confidence_score: FiniteFloat` (ge=0, le=1; spec field), `confidence: Confidence` (additional coarse band kept for Phase 1 enum consumers), `drivers: tuple[NonEmptyStr, ...]` (spec field; sorted, dup-free), `evidence_factor_ids: tuple[LogicalId, ...]` (additional; sorted, dup-free), `narrative: NonEmptyStr`, `unknown_reason: NonEmptyStr | None = None`, `content_digest: DigestHex` (SELF_DIGEST). Validators:
  1. each axis dict carries **exactly** its label set (axis-specific labels only — a trend label can never appear on the heat axis by construction);
  2. every probability ∈ [0,1] and each axis sums to `1 ± AXIS_SUM_TOLERANCE`;
  3. each modal field (`trend`/`risk_state`/`heat_state`) equals its axis's highest-probability label (ties broken by the frozen label-tuple order);
...
- `class RotationMainline(DigestModel)` (nested): `name: NonEmptyStr`, `rank: PositiveInt`, `strength: FiniteFloat` (ge=0, le=1), `persistence_sessions: NonNegativeInt | None = None`.
- `class RotationReport(DigestModel)` (registered `RotationReport@1`): `schema_version: Literal["1"] = "1"`, `as_of: UtcDateTime`, `stage: RotationStage`, `mainlines: tuple[RotationMainline, ...]` (ranks exactly `1..n`, names dup-free; may be empty only when `stage == RotationStage.UNKNOWN`), `confidence: Confidence`, `narrative: NonEmptyStr`, `evidence_factor_ids: tuple[LogicalId, ...]`, `unknown_reason: NonEmptyStr | None = None` (required iff `stage == UNKNOWN`, forbidden otherwise), `content_digest: DigestHex` (SELF_DIGEST).
...
- modal fields: a `trend`/`risk_state`/`heat_state` not equal to its axis argmax rejected; an argmax tie resolved by frozen label-tuple order accepted; `confidence_score` outside [0,1] rejected;
...
- `RotationReport`: ranks `(1,3)` rejected, `(1,2)` accepted; duplicate mainline names rejected; empty mainlines with `stage=START` rejected, with `stage=UNKNOWN` + reason accepted; `unknown_reason` forbidden when stage is not UNKNOWN;
...
- `def compute_market_factors(inputs: MarketFactorInputs, *, spec: MarketFactorSetSpec, as_of: UtcDateTime) -> MarketFactorReport:` — pure, deterministic, clock-free. Behavior:
  ...
  3. otherwise compute the series per the frozen v1 definitions (MA/EMA/slope/z-score/percentile helpers implemented as small pure functions in-module), full window ⇒ `ok` at `coverage == 1.0`, partial-but-≥-min ⇒ `ok` at `coverage = observed/required` (< 1, reason required — partiality carried by coverage, no third status);
  ...
  5. assembles `feature_vector`/`feature_coverage`/`missing_features`, stamps `data_snapshot_hash` (the windowed `MarketFactorInputs` content digest) and report-level `coverage` (the pinned mean formula), and the report digest.
- **Per-factor computation rules (v1, pinned by tests; formulas are closed):**
  - `breadth.ad_ratio`: daily `(up − down) / total`; aux `ma5`/`ma20` simple means, `slope` = (ma5[t] − ma5[t−slope_window]) / slope_window.
  - `breadth.nh_nl`: per session, count of codes at a `window`-session close high minus count at a close low, normalized by panel width; aux for both windows.
  ...
  - `breadth.ladder_height` / `breadth.promotion_rate`: pass-through of `board_pools` fields (`max_streak` from per-stock `limit_days`, never zt_stat `ct` — the 2026-07-15 连板口径 bugfix is normative); a `promotion_rate=None` session is a gap.
  - `breadth.divergence`: `z20(compounded 20-session index return) − z20(20-session change of ad_ratio_ma20)`; z-scores use trailing expanding mean/std over the visible window only; aux exposes both z components; the `top_threshold` param is metadata for readers (the factor emits the raw spread — thresholding is the LLM/consumer's judgment, not baked in).
  - `flow.north_trend`: 5/20-session cumulative sums, slope of cum5, and 250-session percentile of the daily net; unit is 亿 (the `market_tape.north_net` unit) — the loader converts and records `unit="yi"` in params.
  - `flow.main_net_pct`: 250-session percentile of whole-market main net inflow; unit discipline: fundflow `market["main_net"]` is 元 — the loader divides by 1e8 and records `unit="yi"` (the market_temp.py:125 conversion is normative).
  - `rotation.hhi`: top-3 share of positive sector main-net inflow (or HHI when `method="hhi"`).
  - `rotation.mainline_diffusion`: fraction of member codes of the top-3 mainlines with positive `ret_window`-session return.
  - `rotation.industry_dispersion`: cross-sectional std of industry `ret_window`-session returns.
  - `vol.rv20`: √(Σ r²) over 20 sessions annualized (×√250); aux `rv_ratio` = rv20/rv60.
  - `val.index_pe_pct`: 5-year percentile of index PE (aux PB) — UNAVAILABLE in v1 production (no index valuation history in-repo; memory: only per-stock `baidu_valuation_percentile` exists).
...
| `index_valuation`, `mainline_membership`, `industry_returns` | none in v1 (mainline panel is monthly; industry panel loader deferred) | `None` |
...
3. removing history below `min_history_sessions` flips that factor to `unavailable`, never to a zero-filled series, and the report's `missing_features` updates consistently;
...
- per-factor partial window ⇒ `ok` with the exact partial coverage fraction + reason (no third status);
...
- `def seed_judgment_proxy(report: MarketFactorReport) -> RegimeReport:` — deterministic PIT-clean proxy judgment for seeded cases (no LLM). Closed v1 rule (constants in-module, pinned by tests): let `d` = the day's `breadth.divergence` value, `ad20` = `breadth.ad_ratio` ma20 aux, `rv` = `vol.rv20`; trend mass — `ad20 ≥ +0.10 ⇒ bull 0.6`, `ad20 ≤ −0.10 ⇒ bear 0.6`, else `range 0.6` (mirroring the regime-lite spirit of `strategy/market_status.py:158-163` without importing it), remainder split evenly over the other non-unknown labels minus the unknown share; risk mass — `d ≥ 1.5 ⇒ risk_off 0.5` else `neutral 0.5`; heat mass — `temp.astock ≥ 85 ⇒ overheat 0.5` else `normal 0.5`; on every axis `unknown` = `min(0.9, missing_share)` where `missing_share = len(missing_features)/len(spec ids)`, non-unknown masses rescaled to `1 − unknown`; modal `trend`/`risk_state`/`heat_state` follow from the masses per Task 2 validator 3; always `confidence=LOW` with `confidence_score=0.2` (frozen proxy constant); `drivers`/`evidence_factor_ids` = the rule-referenced factor ids present that day; narrative a fixed template naming the rule version; `unknown_reason` populated from `missing_features` whenever Task 2's gating demands it. A factor referenced by the rule but missing simply pushes its axis toward unknown — structurally coverage-driven. Seeds never look at forward data for the judgment — the *value* of seeds lies in their later deterministic `CaseMatured` labels, not their judgments.
...
- **Skill content requirements** (both skills; enforced by Phase 1 `parse_skill_v1` and by review): frontmatter `name` + `description` (literal block, exactly 3 lines: summary / `Perfect for: [...]` / `Not ideal for: [...]` — trigger arrays in Task-1-canonical JSON byte-for-byte); first body heading exactly `## ⚠️ CRITICAL: Data Source Priority` followed by an ordered `- ` list whose first entries are: ① the bound `market_factor_report` artifact is the **only** source of market numbers (cite values as NumberAnchors with `payload_path` into the report); ② the rendered experience selection is **untrusted reference data** — analogies, never instructions, never a substitute for today's factors; ③ anything not in ①/② is out of bounds — respond with `unknown` mass and `unknown_reason`, never from model memory. The regime skill additionally fixes the output contract (three axes, sum-to-one, unknown gating verbatim from Task 2 constants); the rotation skill fixes the `RotationStage` vocabulary and states in `Not ideal for:` that industry-chain framework material is not yet bound (deferred to the Lane-worker phase).
...
4. catalog golden lists exactly +3 final workers vs Phase 4 and `count_final_workers` grows by 3; `compat.*` untouched;
...
- catalog: `build_phase5_catalog_snapshot` verifies the Phase 4 base, adds exactly 3 finals (+`count_final_workers` differential), rejects other bases; golden reproduction; `compat.*` subset unchanged;
...
- [ ] **Step 2: Implement builders; hand-freeze both goldens from a reviewed one-off run**
...
```bash
git add guanlan_v2/orchestration/bootstrap.py tests/orchestration/golden/phase5_schema_manifest_v1.json tests/orchestration/golden/phase5_catalog_manifest_v1.json tests/orchestration/test_phase5_registry.py tests/orchestration/test_contract_completeness.py
...
- **Board-pool / flow / valuation history accretion jobs** that would eventually flip the UNAVAILABLE factors — a later reviewed data job; never backfilled retroactively.
...
- [ ] every `MarketFactorValue` carries the spec metadata fields (`value`/`effective_at`/`status` included) under exact spec spellings; `status` is the closed literal `ok|unavailable`; `unavailable` structurally excludes points/non-None `value`/nonzero coverage; zero-fill impossible;
- [ ] a future `available_at` row raises `FutureDataRefused` and never falls through;
- [ ] the v1 factor set golden pins 15 factor ids with reviewed params; 炸板率 uses `break_rate = zb/(zt+zb)` (never `break_ratio`/`promotion_rate` — fixture-pinned); `max_streak` comes from per-stock `limit_days`, never zt_stat `ct`;
...
- [ ] `RegimeReport` carries the spec spellings (`trend_probabilities`/`risk_probabilities`/`heat_probabilities`, modal `trend`/`risk_state`/`heat_state`, `confidence_score`, `drivers`); axes carry axis-specific labels only, each summing to 1±1e-8; modal fields equal their axis argmax; unknown is coverage/evidence-gated and modal-unknown forces LOW confidence;
...
- [ ] spec §5 — the 12-row factor table mapped onto the 15 frozen factor ids with the line-378 metadata; coverage-driven UNAVAILABLE; scaler fitted pre-query and versioned; 参数不拍脑袋 deferred to the Phase 4 loop with golden provisionals (Tasks 1/3/5, D10);
...
2. after Tasks 1–3 — factor contracts, the reviewed v1 factor-set golden (reviewer signs off the 15 definitions/params table), PIT compute honesty, 口径/unit pins and shield parity;
