...
- `class ShadowContractError(ValueError)` — base; `class ProposalRejected(ShadowContractError)` — stage-前 rejection carrying a closed `reason_code: str` from `{"duplicate_symbol", "non_finite_weight", "negative_weight", "weight_sum_violation", "leverage_or_short"}` (Task 1b extends this closed set with `"non_band_weight"`). (Pydantic `ValidationError` from strict field types also counts as rejection; the model validator raises `PydanticCustomError` with these same reason codes so tests can assert the code.)
- `WEIGHT_SUM_TOLERANCE: float = 1e-8` — module constant; the **only** tolerance in the weight-sum invariant.
- `class TargetPosition(DigestModel)` — `schema_version: Literal["1"] = "1"`; `symbol: Symbol`; `target_weight: FiniteFloat = Field(ge=0, le=1)` (continuous at the `TargetPosition` layer; narrowed to the closed five-band vocabulary `TARGET_WEIGHT_BANDS` by the Task 1b proposal/intent-layer validator — R2 ruling: the trader emits discrete target-weight bands 0/25/50/75/100%, never continuous precise weights; `target_weight` stays **required** — it is the decision substance — while the price/exit-parameter fields below are already Optional-None per the no-computed-defaults rule); `stop_loss_pct: FiniteFloat | None = Field(default=None, gt=0, le=1)`; `take_profit_pct: FiniteFloat | None = Field(default=None, gt=0)`; `max_hold_bars: PositiveInt | None = None`. All fields semantic; no excludes (Task 1b appends `entry_tranches: tuple[TrancheTrigger, ...] = ()`).
- `class PortfolioTargetProposal(DigestModel)` — `schema_version: Literal["1"] = "1"`; `positions: tuple[TargetPosition, ...]`; `cash_weight: FiniteFloat = Field(ge=0, le=1)`; `rationale: NonEmptyStr`; `confidence: Confidence`. Model validator (order fixed): ① duplicate `symbol` (by `Symbol.code + exchange`) → `duplicate_symbol`; ② `abs(sum(target_weight) + cash_weight - 1) > WEIGHT_SUM_TOLERANCE` → `weight_sum_violation` — **never renormalized**; ③ `sum(target_weight) > 1 + WEIGHT_SUM_TOLERANCE` → `leverage_or_short` (long-only v1; shorts are already impossible via `ge=0`, negatives via strict `FiniteFloat` bounds; NaN/Inf via `FiniteFloat`). Task 1b appends step ④ (band-domain check → `non_band_weight`).
...
## Task 1b: Target-weight band vocabulary + tranche-trigger contract

**Files:**
- Modify: `guanlan_v2/orchestration/shadow.py`
- Test: `tests/orchestration/test_shadow_contracts.py`

**Consumes:** Task 1 contracts (`TargetPosition`, `PortfolioTargetProposal`, `ProposalRejected`); Phase 1 `DigestModel`, `FiniteFloat`.

**Produces (R2 ruling 2026-07-18: trader target weights are a closed five-band vocabulary in `@1`; the tranche trigger registers its minimal shape now so Phase 8 never needs an `@2` bump):**

- `TARGET_WEIGHT_BANDS: tuple[float, ...] = (0.0, 0.25, 0.5, 0.75, 1.0)` — module constant, **exported** in the reviewed `__all__`: Phase 8's `allowed_actions` "maximum target-weight band" reuses this exact closed vocabulary — one definition, no parallel copy.
- `PortfolioTargetProposal` model-validator extension (appended to the Task 1 closed order as step ④): every `positions[].target_weight ∈ TARGET_WEIGHT_BANDS` by **exact comparison — zero tolerance, never snapped to the nearest band**; violation → `ProposalRejected` with reason code `"non_band_weight"` (the Task 1 closed reason set extended by exactly this member).
- `class TrancheTrigger(DigestModel)` — frozen minimal-shape sub-model for the optional entry-tranche price interval (R2: trader may attach 分批触发价区间): `price_low: FiniteFloat | None = None`; `price_high: FiniteFloat | None = None`; `fraction: FiniteFloat | None = None`. **All numeric fields Optional, default `None`, and no computed default anywhere**; no cross-field constraints in `@1` (minimal registration — execution semantics arrive with a later matching-engine version); it carries no independent `schema_version` and versions/digests through its host. Attached as `TargetPosition.entry_tranches: tuple[TrancheTrigger, ...] = ()`, thereby entering `TargetPosition@1`'s JSON schema and semantic digest.
- Validator placement is pinned: the band check lives at the **proposal/intent layer** (`PortfolioTargetProposal` here, `TargetPortfolioIntent` in Task 3) and **never in `TargetPosition` itself** — `TargetPosition` stays continuous so the Task 6 deterministic lane (`DeterministicTargetSet`) can express continuous rule-computed weights; the band constraint targets the LLM anchor-drift pathology only (rationale mirrored in Task 6).

**Required invariants:**

1. band domain is exact and closed: every value in `TARGET_WEIGHT_BANDS` passes; `0.3` (and any other off-band value) raises `ProposalRejected` with observable reason `"non_band_weight"` — no snapping, no tolerance;
2. a `TrancheTrigger` with all fields `None` is valid and constructible; no field acquires a computed default on any path;
3. digest sensitivity: two otherwise-identical positions differing only in an `entry_tranches` entry have different semantic digests; the default `()` leaves Task 1 digest vectors meaningful;
4. `TARGET_WEIGHT_BANDS` is exported, immutable (tuple) and is the value the validator reads — no second in-module literal copy of the band values exists.

- [ ] **Step 1: Write failing band/tranche tests**

Cover the band boundary matrix (each band value accepted; `0.3` rejected with the observable reason code), all-`None` tranche legality, digest sensitivity, and the export surface.

Run now: `pytest tests/orchestration/test_shadow_contracts.py -v`

Expected: FAIL on missing `TARGET_WEIGHT_BANDS`/`TrancheTrigger`/band validator.

- [ ] **Step 2: Implement the band validator + `TrancheTrigger` in `shadow.py`**

- [ ] **Step 3: Run and commit**

Run: `pytest tests/orchestration/test_shadow_contracts.py -v` — Expected: PASS.

```bash
git add guanlan_v2/orchestration/shadow.py tests/orchestration/test_shadow_contracts.py
git commit -m "feat(orchestration): closed target-weight bands + minimal TrancheTrigger in phase6 @1"
```
...
- **Deterministic dual-curve lane (reviewed additive runner entry, ruled 2026-07-16 — this plan owns the runner; Phase 9's dual-curve replay is the consumer):**
  - `class DeterministicTargetSet(ContractModel, frozen)` (internal carrier, classified per the firewall mechanism): `rule_id: NonEmptyStr`; `point_ordinal: NonNegativeInt`; `target_version: PositiveInt`; `session_date: IsoDateStr` (the decision point); `positions: tuple[TargetPosition, ...]`; `cash_weight: FiniteFloat = Field(ge=0, le=1)` — validated by the exact Task 1 duplicate/weight-sum/leverage matrix, **exempt from the Task 1b band-domain check** (explicit exemption: anchor drift is an LLM pathology; the deterministic rule leg must be able to express continuous weights such as equal-weight-three ≈ 33.3% — which is exactly why Task 1b pins the band validator at the proposal/intent layer and not in `TargetPosition` itself, which both lanes inherit). Produced **without** any LLM or intent envelope: it carries no `origin`/`authority`/`execution_scope` and can never become a `TargetPortfolioIntent`.
...
- `PHASE6_PUBLIC_MODELS: tuple[type, ...]` — exactly the ten registered Phase 6 payloads: `TargetPosition`, `PortfolioTargetProposal`, `TargetPortfolioIntent`, `DecisionSchedule`, `ShadowTargetApplyRecord`, `ShadowOrderRecord`, `ShadowFillRecord`, `ShadowRejectRecord`, `ShadowRunResult`, `CorporateActionEvent`. `TrancheTrigger` (Task 1b) is a nested frozen sub-model that enters the registry through `TargetPosition@1`'s JSON schema and digest — it is not independently registered unless the implemented upstream classification mechanism requires nested sub-models to be enumerated, in which case resolve it per the Task 0 correction clause (adopt the reviewed mechanism, never invent parallel semantics).
...
- Golden `phase6_schema_manifest_v1.json` — full manifest of the sealed Phase 6 registry (base digest + every `SchemaRef` + JSON-schema digest), hand-frozen, never regenerated from test code (the manifest reflects whichever reviewed `TrancheTrigger` classification outcome applies — nested-in-`TargetPosition@1` by default). Golden `phase6_catalog_manifest_v1.json` — records `{base: PHASE5_CATALOG_DIGEST, result: PHASE6_CATALOG_DIGEST, workers_added: 0, capabilities_added: 0}`.
...
8. **no defaults minted at any extraction layer** (Task 1b tranche discipline, structural): a proposal whose optional numeric fields are all `None` (`stop_loss_pct`/`take_profit_pct`/`max_hold_bars` and every `TrancheTrigger` field) wraps into an intent that is still all-`None`, and running it produces **zero** stop/take/max-hold orders and zero tranche behavior — no code path anywhere replaces a `None` with a computed price (the banned counter-example: defaulting a buy trigger to 现价×1.15); `None`-fidelity is asserted across all three layers proposal → intent → runner.
