...
**Files:**
- Modify: `guanlan_v2/orchestration/pipeline/live_decide.py` (the registration block only), `tests/orchestration/test_pm_two_bridges.py`, `tests/orchestration/data/test_data_catalog.py` — the incumbent class itself is deleted in Task 5, when its last caller (the per-run view) is re-targeted
...
```bash
git add guanlan_v2/orchestration/pipeline/live_decide.py guanlan_v2/orchestration/adapters/data_world.py tests/orchestration/test_pm_two_bridges.py tests/orchestration/data/test_data_catalog.py tests/orchestration/test_production_data_provider.py
