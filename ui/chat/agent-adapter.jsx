...
class GuanlanAgent {
  ...
  async run(query, sessionContext, cb, opts = {}) {
    if (this.backendUrl) {
      // 连了后端就只认后端: 失败直接抛给 UI (onError → 状态栏「⚠ LLM 失败」),
      // 不再偷偷回退 mock —— 那会用假数据掩盖真实故障 (端口没起 / 引擎报错 / 无 LLM key),
      // 让人误以为在看真数据。失败要显形, 才能去修后端。
      try {
        return await this._runBackend(query, sessionContext, cb, opts);
      } catch (e) {
        if (this.cancelled) return cb.onCancel();
        console.error('[guanlan] 后端 SSE 失败:', e);
        return cb.onError(e instanceof Error ? e : new Error(String(e)));
      }
    }
    // 没配后端 (file:// 直开 / 显式留空) 才走内置 mock 预览。
    return this._runMock(query, sessionContext, cb);
  }

  // ─── 真后端 ───
  async _runBackend(query, sessionContext, cb, { sessionId, mode = 'auto', model = null, profile = null, history = null } = {}) {
    ...
    const resp = await fetch(`${this.backendUrl}/run`, {
      ...
      body: JSON.stringify({
        ...
        profile: profile || null,    // 模块工具域: 对话端='research' → agent 调不到因子类工具
        history: history || null,    // 客户端近期对话 → 后端 seed 冷 agent (抗进程重启失忆)
      }),
