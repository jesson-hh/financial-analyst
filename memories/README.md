...
5. **不进 source 的 staging**: `_pending_introspections/`, `_proposed/`,
   `_shared/conversation_lessons.md` 是 runtime 生成的, 已 `.gitignore`.
...
- ❌ **不要把私人 personal data push 上去**: `_shared/conversation_lessons.md` 已 ignore, 个人 /lesson 不出现在公开 fork.
...
5. **Runtime staging not committed**: `_pending_introspections/`, `_proposed/`, `_shared/conversation_lessons.md` are runtime-generated, `.gitignore`'d.
...
- ❌ Don't push personal data — `_shared/conversation_lessons.md` is gitignored; personal `/lesson` stays local.
